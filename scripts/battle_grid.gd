...
func _draw():
	draw_rect(Rect2(Vector2.ZERO, Vector2(GRID_COLS * CELL_SIZE, GRID_ROWS * CELL_SIZE)), Color.DARK_GRAY)
	var tex_scale = 0.35
	for row in range(GRID_ROWS):
		for col in range(GRID_COLS):
			var cell = grid[row][col]
			draw_rect(Rect2(cell.grid_pos * CELL_SIZE, Vector2(CELL_SIZE, CELL_SIZE)), Color.BLACK, false, 2)
			if cell.type:
